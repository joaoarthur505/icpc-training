...
int main() {
	...
	forn(i, 0, n){
		...
		if(cmp(norm(center[i]), radius[i]) <= 0) secure = true;
	}
	...
	forn(i, 0, n){
		ld divise = sqrt(r*r + radius[i]*radius[i]);
		pt pl, pr;

		ld r_aux = r;
		if(cmp(norm(center[i]), divise) <= 0) r_aux = sqrt(norm_sq(center[i]) - radius[i]*radius[i]);

		auto aux = circint(pt(0,0), r_aux, center[i], radius[i]);
		if(aux.second == pt(DINF,DINF)) continue;
		pl = aux.first;
		pr = aux.second;
		if(ccw(pt(0,0), pr, pl)) swap(pl, pr);

		trace(
			cout << fixed << setprecision(10) << pl.x << " " << pl.y << ln;
			cout << ln;
			cout << fixed << setprecision(10) << pr.x << " " << pr.y << ln;
			cout << ln;
		);

		ld left = angle(pl), right = angle(pr);
		sweepline[left]++;
		sweepline[right]--;
		if(right < left){
			sweepline[0]++;
	...
	cout << fixed << setprecision(10) << (sum/(2*pi)) << ln;
