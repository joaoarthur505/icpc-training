...
int main(){
    _;
    ll n, s, e; cin >> n >> s >> e;
    dinitz fluxo(2*n + 1 + 1, true);
    // 2*i é entrada do vertice i
    // 2*i+1 é saida do vertice i
    ll source = 2*n;
    ll tink = 2*n+1;
    ...
    forn(i,0,s){
        ll x; cin >> x;
        x--;
        fluxo.add(source,2*x,1);
    }
    
    forn(i,0,e){
        ll x; cin >> x;
        x--;
        fluxo.add(2*x+1, tink, 1);
    }

    forn(i,0,n) fluxo.add(2*i, 2*i+1, 1);
    ...
    forn(i,0,m){
        ...
        cin >> a >> b;
        a--, b--;
        fluxo.add(2*a+1, 2*b, 1);
    }

    cout << fluxo.max_flow(source,tink) << ln;
}