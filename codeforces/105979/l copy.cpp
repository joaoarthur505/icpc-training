...
void dfs_fim(map<ll,ll>& mp, ll i, ll l, ll to_close, ll acc){
    ...
    if(par[i] == 0) to_close = max(to_close-1,0ll);
    else to_close++;
    
    if(to_close == 0) mp[acc]++;

    for(ll j : g[i]) if (j != l && !rem[j]) dfs_ini(mp, j, i, to_close, acc); 
}
...
ll decomp(ll i){
    ...
    for(ll j : g[c]) if (!rem[j]) {
        ...
        for(ll t : g[j]) if (!rem[j]) {
            map<ll,ll> lini;    
            map<ll,ll> lfim; 
            dfs_ini(lini, t, j, 0, 0);
            dfs_fim(lfim, t, j, 0, 0);
